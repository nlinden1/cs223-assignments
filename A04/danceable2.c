...
//method that iterates through the linked list and sets printme to true
void printall() {
   struct node *thisnode;
   for(thisnode = head; thisnode != NULL; thisnode = thisnode->next) {
      thisnode->d.printme = true;
   }
}
...
bool fakedelete(struct dysong s) {
   ...
   if(current == head) {
      head->d.printme = false;
   } else {
      current->d.printme = false;
   }    
